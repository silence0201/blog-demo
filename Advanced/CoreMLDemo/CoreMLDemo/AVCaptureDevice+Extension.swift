...
extension AVCaptureDevice {
    private func availableFormatFor(preferredFps: Float64) -> [AVCaptureDevice.Format] {
    ...
    private func formatWithHeighestResolution (_ availableFormats: [AVCaptureDevice.Format]) -> AVCaptureDevice.Format? {
    ...
    func updateFormatWithPreferredVideoSpec(preferredSpec: VideoSpec) {
        ...
        if let preferredFps = preferredSpec.fps {
            availableFormats = availableFormatFor(preferredFps: Float64(preferredFps))
        }else {
        ...
        if let preferredSize = preferredSpec.size {
            ...
            selectedFormat = formatWithHeighestResolution(availableFormats)
        }
